// =============================================
// NAVBAR + BUSCADOR UNIFICADO - LZPLAY (v3.3)
// Cambios: 
// - Grid responsive
// - Orden por año (nuevo → viejo)
// - Normalización de tildes y caracteres especiales
// - Scroll arreglado en el modal del buscador
// =============================================
...
.lz-modal-content {
    ...
    border-radius: 24px;
    max-height: 85vh;           /* ← Reducido para mejor scroll */
    overflow-y: auto;           /* ← Scroll principal aquí */
    overflow-x: hidden;
...
.lz-close-btn:hover { background: rgba(255,255,255,.15); color: white; }

.lz-search-area { padding: 20px 22px; }
...
@media (max-width: 900px) {
    ...
    .lz-modal-content { max-height: 82vh; }
    .lz-results-grid {
...
function renderModalResults(query) {
  ...
  grid.innerHTML = results.map(item => `
    <div class="lz-result-card" onclick="window.openContent('${item.url}')">
      <img src="${item.poster || 'https://via.placeholder.com/300x450/111827/ffffff?text=LZPLAY'}" loading="lazy" alt="${item.titulo}">
...
async function initSearch() {
  ...
  desktopIcon?.addEventListener("click", () => window.openSearchModal(desktopInput.value.trim()));
  ...
  mobileBtn?.addEventListener("click", (e) => { e.preventDefault(); window.openSearchModal(); });

  modalInput?.addEventListener("input", () => {
    const query = modalInput.value.trim();
    document.getElementById("resultsTitle").textContent = query ? `Resultados de: "${query}"` : "Buscar contenido";
    renderModalResults(query);
  });

  modalInput?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") renderModalResults(modalInput.value.trim());
  });

  console.log("%c✅ Navbar + buscador listo (v3.3 - Scroll arreglado)", "color:#22c55e;font-weight:bold");
}
